...
from models import User, Question, Chapter, Test, TestQuestion, TestResult, QuestionAnswer
from models import UserRole, QuestionDifficulty, QuestionType
...
def teacher_students():
    ...
    student_data = []
    for student in students:
        test_results = TestResult.query.filter_by(student_id=student.id, completed=True).all()
        
        # Calculate metrics
        tests_taken = len(test_results)
        if tests_taken > 0:
            avg_score = sum(result.total_score for result in test_results) / tests_taken
            max_score = max((result.total_score for result in test_results), default=0)
        else:
            avg_score = 0
            max_score = 0
...
@app.route('/teacher/manage_questions', methods=['GET', 'POST'])
@login_required
...
def create_test():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('add_questions_to_test', test_id=test.id))
    
    return render_template('teacher/create_test.html', title='Create Test', form=form)


@app.route('/teacher/test/<int:test_id>/add_questions', methods=['GET', 'POST'])
@login_required
...
def student_dashboard():
    ...
    if total_tests_taken > 0:
        avg_score = sum(result.total_score for result in completed_tests) / total_tests_taken
...
@app.route('/student/create_test', methods=['GET', 'POST'])
@login_required
def student_create_test():
    ...
    if form.validate_on_submit():
        ...
        if len(available_questions) < form.num_questions.data:
            ...
            return redirect(url_for('student_create_test'))
...
def start_test(test_id):
    ...
    if existing_result:
        return redirect(url_for('take_test', result_id=existing_result.id))
    ...
    if completed_result:
        ...
        return redirect(url_for('test_results', result_id=completed_result.id))
...
@app.route('/student/take_test/<int:result_id>', methods=['GET', 'POST'])
@login_required
...
@app.route('/student/test_results/<int:result_id>')
@login_required
